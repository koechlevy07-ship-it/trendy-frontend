/**
 * Validation Middleware - Chapter 11 Part 4
 * 
 * Executes DTO validation using class-validator and class-transformer.
 * Normalizes payloads and rejects malformed requests before business logic.
 */
...
export class ValidationMiddleware implements NestMiddleware {
  ...
  use(req: Request, res: Response, next: NextFunction): void {
    const dtoClass = this.reflector.get(VALIDATION_DTO_KEY, req.route?.stack[req.route?.stack.length - 1]?.handle);
