...
export class FacilityAccessControl {
  ...
  authorizedOrganizations: Types.ObjectId[];
...
VenueFacilitySchema.index({ facilityName: 'text', description: 'text' });
VenueFacilitySchema.index({ facilityId: 1 }, { unique: true });
VenueFacilitySchema.index({ venueId: 1, facilityType: 1 });
VenueFacilitySchema.index({ venueId: 1, status: 1 });
VenueFacilitySchema.index({ facilityType: 1, status: 1 });
VenueFacilitySchema.index({ facilityId: 1 }, { unique: true });
VenueFacilitySchema.index({ 'schedule.operatingDays': 1 });
...
VenueFacilitySchema.virtual('isOperational').get(function() {
  return this.status === FacilityStatus.ACTIVE && this.availability === FacilityAvailability.AVAILABLE;
});